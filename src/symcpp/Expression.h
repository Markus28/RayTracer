...
public:
    ...
    virtual std::unique_ptr<Expression> simplify() = 0;     //Calling simplify might corrupt the original Expression. Only use the result of simplify()
    virtual bool is_constant() const = 0;
    virtual unsigned int precedence() const = 0;
    virtual double const_eval() const = 0;   //If this method is called assert(is_constant()) must hold. The Expression is constant and the value is returned
