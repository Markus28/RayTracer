...
Multiply::Multiply(std::unique_ptr<Expression>&& a, std::unique_ptr<Expression>&& b): a(std::move(a)), b(std::move(b)) {}
...
bool Multiply::is_constant() const {
    ...
    return (a->is_constant()&&b->is_constant()) || (a->is_constant()&&a->const_eval()==0) || (b->is_constant()&&b->const_eval()==0);
}
...
double Multiply::const_eval() const {
    ...
    return b_val*a_val;     //a_val and b_val must have been intialized, otherwise assert(is_constant) would have failed
}