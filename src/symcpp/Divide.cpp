...
namespace symcpp {
    ...
    Divide::Divide(const std::unique_ptr<Expression> &a, const std::unique_ptr<Expression> &b) : a(a->copy()),
                                                                                                 b(b->copy()) {}
    ...
    std::unique_ptr<Expression> Divide::diff(std::string v) const {
        std::unique_ptr<Expression> p(
                new Divide(Subtract(Multiply(a->diff(v), b->copy()), Multiply(a->copy(), b->diff(v))),
                           Multiply(b->copy(), b->copy())));
        return p;
    }
