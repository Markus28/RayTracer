...
Power::Power(const std::unique_ptr<Expression> &a, const std::unique_ptr<Expression> &b): a(a->copy()), b(b->copy()) {}
...
std::unique_ptr<Expression> Power::diff(std::string var) const {
    if(b->is_constant()){
        ...
        std::unique_ptr<Expression> p (new Multiply(a->diff(var), std::unique_ptr<Expression>(new Multiply(Constant(b_val), Power(*a, Constant(b_val-1.0))))));
        return p;
    }

    //General derivative:
    std::unique_ptr<Expression> p (new Multiply(Add(Multiply(b->diff(var), std::unique_ptr<Expression>(new Logarithm(*a))), Multiply(b->copy(), Logarithm(*a).diff(var))), Power(a, b)));
    return p;
}
